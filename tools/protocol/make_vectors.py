...
EXAMPLES = [
    ...
    ("config_set_max_tx", "CONFIG_SET", 12,
     {"flags": 1, "config": {"key": "MAX_TX_S", "selector": 0, "value": {"s": 300}}},
     "Maximum continuous TX 300 s (the default)."),
    ("config_set_max_tx_long", "CONFIG_SET", 43,
     {"flags": 1, "config": {"key": "MAX_TX_S", "selector": 0, "value": {"s": 86400}}},
     "Maximum continuous TX 24 h: there is no upper bound."),
    ("config_set_max_tx_disabled", "CONFIG_SET", 45,
     {"flags": 1, "config": {"key": "MAX_TX_S", "selector": 0, "value": {"s": 0}}},
     "Maximum-TX timer disabled by the user (0)."),
    ...
    ("config_set_power_down_delay", "CONFIG_SET", 42,
     {"flags": 1, "config": {"key": "POWER_DOWN_DELAY_S", "selector": 0, "value": {"s": 30}}},
     "Power down 30 s (the default) after the radio or ignition turns off."),
    ("config_set_power_down_never", "CONFIG_SET", 44,
     {"flags": 1, "config": {"key": "POWER_DOWN_DELAY_S", "selector": 0, "value": {"s": 0}}},
     "Never power down automatically (user's choice; variant M may exceed its off-state drain target)."),
    ("config_set_wired_port_lock", "CONFIG_SET", 53,
     {"flags": 1, "config": {"key": "WIRED_PORT_LOCK", "selector": 0, "value": {"level": 1}}},
     "Native serial ports stay, but their RTS/DTR never key PTT."),
    ("config_set_wired_port_lock_isolate", "CONFIG_SET", 54,
     {"flags": 1, "config": {"key": "WIRED_PORT_LOCK", "selector": 0, "value": {"level": 3}}},
     "No native serial ports, and the radio's own USB devices are isolated from the host."),
    ...
    ("ptt_status_keyed", "PTT_STATUS", 24, {"state": 1, "sources": 0x01, "reason": 0, "remaining_s": 300},
     "Keyed by the protocol command; 300 s of TX left."),
    ...
    ("ptt_status_watchdog", "PTT_STATUS", 0,
     {"state": 0, "sources": 0x00, "reason": 12, "remaining_s": 0},
     "Reported after a watchdog reset: the ESP32-S3 watchdog reset the device after a lock-up; PTT went off."),
